...
const PrCard = ({ pr, me }: { pr: MyPr; me: string }) => (
  <CardFrame
    title={pr.title}
    author={me}
    repo={pr.repo}
...
const Column = ({ title, prs, me }: { title: string; prs: MyPr[]; me: string }) => (
  <div className="flex min-h-0 flex-1 flex-col gap-2 rounded-lg bg-grass-600/10 p-2">
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto">
      {prs.map((pr) => (
        <PrCard key={pr.id} pr={pr} me={me} />
      ))}
    </div>
  </div>
)

export const PullRequests = ({ prs, me }: Props) => {
  ...
  return (
    <div className="flex h-full flex-col gap-3">
      ...
      <div className="flex min-h-0 flex-1 gap-3">
        {COLUMNS.map((col) => (
          <Column key={col.column} title={col.title} prs={byColumn(col.column)} me={me} />
        ))}
        {showDone && <Column title="Done" prs={byColumn('done')} me={me} />}
      </div>
    </div>
  )
}
